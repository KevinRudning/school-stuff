...
class account:
    ...
    def createPin(self):
        print('Please choose a pin code for this account. The pin should be between 4 and 8 digits. If you no longer wish to choose this account, input the number 0')
    ...
    def loginPin(self):
        print('Please enter your Pin code. If you wish to change accounts, enter the number 0.')
    ...
    def cmd(self):
        ...
        func = int(input('''What would you like to do with this account?
    1: Deposit
    2: Withdraw
    ...
    def deposit(self):
        ...
        self.TXNhist.append('Deposited ' + str(amount) + ', balance is ' + str(self.balance) + '.')
        self.cmd()
    ...
    def withdraw(self):
        ...
        if self.balance >= amount:
            self.balance -= amount
            self.TXNhist.append('Withdrew ' + str(amount) + ', balance is ' + str(self.balance) + '.')
    ...
    def transfer(self):
        ...
        if self.balance >= amount:
            ...
            if reci <= 0:
                ...
                self.TXNhist.append('Transferred ' + str(amount) + ' to account number ' + str(reci) + ', balance is ' + str(self.balance) + '.')
                accounts[reciOut].TXNhist.append('Received ' + str(amount) + ' from account number ' + str(accNrIn) + ', balance is ' + str(accounts[reciOut].balance) + '.')
                print('You transferred: ' + str(amount) + ' to account number ' + str(reci))    
        else:
            print("Insufficient balance")
        self.cmd()
    ...
    def nameFunc(self):
        ...
        if self.nameAssigned == False:
            ...
            self.nameCMD = int(input('This account has already been named "' + self.name + '''", would you like to rename it?
            1: Yes
            2: No
            3: Remove account name
            '''))
            if self.nameCMD == 1:
                print('You are currently renaming your account. What should the new name be?')
                self.newName = str(input())
...
print('''Hello, and welcome to "A Bank With A Very Creative Name"!
...
while True:
    ...
    cmd = int(input('''What would you like to?
    1: Add new account
    ...
    if cmd == 1:
        ...
        for i in range(len(accounts)):
            if accounts[i].name == 'Unnamed':
                print('Account number ' + str(i+1) + ' is unnamed, with a balance of ' + str(accounts[i].balance))
            else:
                print('Account number ' + str(i+1) + ' is named ' + accounts[i].name + ', with a balance of ' + str(accounts[i].balance))
    elif cmd == 3:
